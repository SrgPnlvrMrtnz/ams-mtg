...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	const cardNames: string[] = Array.isArray(body?.cards) ? body.cards : [];
	...
	if (!tagCounts['bajo-coste']) alerts.push('Tu mazo carece de jugadas tempranas.');

	const highCostPct = ((tagCounts['alto-coste'] ?? 0) / total) * 100;
	if (highCostPct > 50) {
		alerts.push(
			`Demasiadas cartas de alto coste (${Math.round(highCostPct)}%). Considera añadir más ramp.`
		);
	}
