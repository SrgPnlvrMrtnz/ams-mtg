<script lang="ts">
	...
	async function send() {
		...
		try {
			...
			messages = [...messages, { role: 'assistant', content: `⚠️ Error de conexión: ${err.message}` }];
		} finally {
			loading = false;
		}
	}
...
<style>
	...
	.clear-btn:hover { background: #1c1c28; color: #f0eff6; border-color: #5a596e; }
	...
	.send-btn:hover:not(:disabled) { background: #9b7cff; }
	.send-btn:active:not(:disabled) { transform: scale(0.95); }
	.send-btn:disabled { background: #3a2a7a; opacity: 0.5; cursor: not-allowed; }
</style>
